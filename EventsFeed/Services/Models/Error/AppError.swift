...
extension Error {
    func toAppError(context: String = "", serviceType: MusicServiceType? = nil) -> AppError {
        ...
        default:
            // Конвертуємо будь-яку іншу помилку в AuthError, якщо вона пов'язана з авторизацією
            if let nsError = self as NSError?,
               nsError.domain.contains("auth") || nsError.domain.contains("Auth") ||
               nsError.domain.contains("authentic") {
                return .auth(description: errorDescription)
            }
